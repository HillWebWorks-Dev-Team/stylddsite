-- Review submit: ignore client-provided name; use booking name or Anonymous.

drop function if exists public.styld_tenant_submit_review(text, text, integer, text, text);
...
grant execute on function public.styld_tenant_submit_review(text, text, integer, text, text, boolean) to anon, authenticated;
